...
public class CheckValidation
{
	...
	public boolean registerData(UserBookStore user) throws InvalidDataException
	{
		...
		if (!user.getLastName().matches(regex)) throw new InvalidDataException("Last name must contain only alphabets");
		regex = "^[\\w-_\\.+]*[\\w-_\\.]\\@([\\w]+\\.)+[\\w]+[\\w]$";
		if (!user.getEmailId().matches(regex)) throw new InvalidDataException("Wrong input for email Id");
		regex = "(0/91)?[7-9][0-9]{9}";
		if (!user.getMobileNumber().matches(regex))
